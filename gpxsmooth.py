...
def extract(trkpt):
    return (
        math.pi / 180 * float(trkpt.attrib["lat"]),
        math.pi / 180 * float(trkpt.attrib["lon"]),
    )
...
def dist(p0, p1):
    a = (
        math.sin((p1[0] - p0[0]) / 2) ** 2
        + math.cos(p0[0]) * math.cos(p1[0]) * math.sin((p1[1] - p0[1]) / 2) ** 2
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: %s <gpx file> <maximum distance> <maximum interval>" % sys.argv[0]
        )
        sys.exit(-1)
    ...
    print(
        "Note: this script will also strip elevation data, assuming it to be inaccurate"
    )
    for pt in trkseg:
